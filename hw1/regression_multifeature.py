...
smooth = 0.00000005
if ("weight_mf.npy" in npy_files) and ("bias_mf.npy" in npy_files) and ("weight_ada_mf.npy" in npy_files) and ("bias_ada_mf.npy" in npy_files):
	weight = np.load("weight_mf.npy")
	bias = np.load("bias_mf.npy")
	weight_adam = np.load("weight_ada_mf.npy")
	bias_adam = np.load("bias_ada_mf.npy")
else:
	...
	weight_adam = np.array([[smooth for x in xrange(time)] for y in xrange(data_feature)])
	bias_adam = 0.0 + smooth # useless
...
def generating_valid():
	...
	for z in xrange(batch_number):
		...
		residual = label - Validset[9][rand_[z] + time - 1]
		loss = loss + pow( (np.multiply(weight,batch[z]).sum() + bias - residual),2 )
...
for j in xrange(epoch): 
	...
	for i in xrange(step):
		...
		for z in xrange(batch_number):
			...
			residual = label - Dataset[9][rand_[z]+time-1]
			temp = 2*(np.multiply(weight,batch[z]).sum() + bias - residual)
		...
		# update new bias
		bias = bias - learning_rate/bias_adam*(grad_b + 2*reg*bias)
	...
	np.save('weight_ada_mf.npy', weight_adam)
	np.save("bias_mf.npy", bias)
	np.save("bias_ada_mf.npy", bias_adam)
...
with open(filename, 'wb') as csvfile:
	...
	for x in xrange(test_number):
		...
		for i in xrange(data_feature):
			for j in xrange(time):
				test_data[i][j] =  (test_data[i][j] - data_normalize[i][0]) / data_normalize[i][1]
		########## To change
		predict = ((np.multiply(weight,test_data).sum() + bias) + test_data[9][time - 1])*PM_var + PM_mean 
		##########
