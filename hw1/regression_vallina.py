...
if ("weight_val.npy" in npy_files) and ("bias_val.npy" in npy_files):
	weight = np.load("weight_val.npy")
	bias = np.load("bias_val.npy")
else:
...
def generating_valid():
	...
	for z in xrange(batch_number):
		...
		loss = loss + pow( (np.multiply(weight[9],batch[z][9]).sum() + bias - label),2 )
	loss = loss/batch_number
	return loss

# set training condition
learning_rate = 0.0000005 ####################################################adjust learning rate
# reg = 10
step = 200
batch_number = 40
epoch = 6000
...
if ("info_val.npy" in npy_files):
	info = np.load("info_val.npy")
else: 
	info = np.zeros((1,2))
# training
# Dataset = Data_norm
for j in xrange(epoch):
	...
	for i in xrange(step):
		...
		for z in xrange(batch_number):
			...
			temp = 2*(np.multiply(weight[9],batch[z][9]).sum() + bias - label)
			grad_b = grad_b + temp/batch_number
			for y in xrange(time):
				current_grad = temp*batch[z][9][y]/batch_number 
				grad_w[9][y] = grad_w[9][y] + current_grad # + 2*reg*weight[x][y]
		...
		# updatw new weight
		for y in xrange(time):
			weight[9][y] = weight[9][y] - learning_rate*grad_w[9][y] 
		# test
		loss = 0
		for z in xrange(batch_number):
			label = Dataset[9][rand_[z]+time]
	...
	np.save("weight_val.npy", weight)
	np.save("bias_val.npy", bias)
	np.save("info_val.npy", info)
	if (test_Loss < 2):
		print ("Done!!!!!!!!!")
...
with open(filename, 'wb') as csvfile:
	...
	for x in xrange(test_number):
		...
		########## To change
		predict = (np.multiply(weight[9],test_data[9]).sum() + bias) 
		##########
